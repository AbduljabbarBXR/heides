...
Taint is traced inside one function block at a time. A variable assigned from a source line is tainted for the rest of that block. A sink line reports critical when it uses a tainted variable, or when a qualifying source exists earlier in the same block at the same or deeper indent within the last 60 lines. Sources are the standard user entry points per language, request parameters, form values, headers, cookies, query strings, environment variables, storage reads. Sinks are SQL execution, shell and system execution, file system writes, dynamic code evaluation, prompt construction. All data sink findings report critical. The reported source line is 0 when the trace is indirect, meaning the source exists in the block but the sink does not name the tainted variable itself.
