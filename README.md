...
0.10.0 widened the dependency guard beyond cargo and npm. go.mod, requirements.txt, pyproject.toml, pom.xml, composer.lock and composer.json all feed the same OSV vulnerability check and the latest release comparison, with the go proxy, pypi, maven central and packagist answering the latest version queries. OSV queries run only on pinned versions, a range requirement never claims a vulnerability it cannot prove.
